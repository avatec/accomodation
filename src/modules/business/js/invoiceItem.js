...
/**
	...
	$.itemAddSession = function() {
		...
		$.ajax({
			url: "/ajax/business/invoiceAddItem/",
			method: "post",
			data : postdata,
			beforeSend: function() {
				$("#btnAddItem").hide();
	...
	$.itemsRefresh = function() {
		...
		$.ajax({
			url: "/ajax/business/invoiceGetItem/",
			method: "post",
			data : {  },
			success: function(data) {
				if( data ) {
...
$.itemDelete = function( key, id ) {
	...
	$.ajax({
		url: "/ajax/business/invoiceDeleteItem/",
		method: "post",
		data : { key: key, id: id },
		beforeSend: function() {
			//$(".fa-spin").fadeIn();
	...
	$.itemsView = function( obj ) {
		...
		for( var i=0; i<=num; i++ ) {
			if( obj[i] ) {
				...
				html += '<td><button type="button" class="btn btn-danger btn-sm itemDelete" data-key="' + i + '"><span class="fa fa-remove"></span></button></td>';
				html += '</tr>';
			}
		}
		$("#invoiceAddInput .item-view").remove();
