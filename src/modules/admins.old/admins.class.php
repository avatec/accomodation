...
class Admins
{
	...
	public static $types = [
		["id" => "1", "name" => "Administrator główny"],
		["id" => "2" , "name" => "Moderator strony"]
	];
	...
	{
		global $config, $admin_folder, $route, $request;
	...
	protected function install()
	{
		if( Db::has_table( self::$table ) == false ) {
			Db::install("CREATE TABLE " . self::$table . " (`id` int(11) UNSIGNED NOT NULL,`login` varchar(200) NOT NULL,`password` varchar(200) NOT NULL,`name` varchar(200) NOT NULL,`email` varchar(200) NOT NULL,`type` tinyint(1) UNSIGNED NOT NULL,`state` enum('FALSE','TRUE') NOT NULL,`create_date` datetime NOT NULL,`last_login_date` datetime DEFAULT NULL,`error_login_date` datetime DEFAULT NULL,`error_login_ip` varchar(30) DEFAULT NULL,`access` text,`avatar` varchar(200) DEFAULT NULL) ENGINE=MyISAM DEFAULT CHARSET=utf8;");
			Db::install("ALTER TABLE " . self::$table . " ADD PRIMARY KEY (`id`);");
			Db::install("ALTER TABLE " . self::$table . " MODIFY `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;");
		}
	}
 ...
 */
	...
	public function login( $login = null, $password = null )
	{
		...
		Kernel::log("admin_login_history.log" , $login . " [" . Common::get_ip() . "] dnia " . date('Y-m-d') . " o godz. " . date('H:i:s'));

		self::store();
		Kernel::setMessage("NOTICE" , "Logowanie przebiegło pomyślnie");
		return true;
	}
	...
	{
		global $app_path;
		if( System::file_exists( $app_path . 'cache/login.log' ) == false ) {
			return false;
		}
 ...
 */
	...
	public function change_password( $id = null )
	{
		...
		Kernel::setMessage("NOTICE" , "Hasło zostało zmienione. Na adres e-mail " . $r['email'] . " została wysłana informacja potwierdzająca tą operację.");
		return true;
	}
	...
	public function activate( $id )
	{
		if(empty(self::$auth['type']) || self::$auth['type'] != 1 ) {
			Kernel::setMessage("ERROR" , "Nie masz wystarczających uprawnień, aby aktywować konto");
			return false;
		}
		...
		Kernel::setMessage("NOTICE" , "Konto zostało pomyślnie aktywowane. Na adres e-mail " . $r['email'] . " została wysłana informacja potwierdzająca tą operację.");
		return true;
	}
	...
	{
		if(empty(self::$auth['type']) || self::$auth['type'] != 1 ) {
			Kernel::setMessage("ERROR" , "Nie masz wystarczających uprawnień, aby dezaktywować konto");
			return false;
		}
		...
		Kernel::setMessage("NOTICE" , "Konto zostało pomyślnie dezaktywowane. Na adres e-mail " . $r['email'] . " została wysłana informacja potwierdzająca tą operację.");
		return true;
	}
	...
	public function delete( $id )
	{
		...
		Kernel::setMessage("NOTICE" , "Konto zostało pomyślnie usunięte");
	}
 ...
 */
	...
	public function update_access( $id )
	{
		...
		if( $r == true ) {
			Kernel::setMessage("NOTICE" , "Uprawnienia zostały zapisane");
			return true;
		}
 ...
 */
	...
	protected static function send_message( $type, $data )
	{
		...
		$smarty->assign("config" , $config = array_merge($config, $system->configGet()));
		$smarty->assign("app_url" , $app_admin_url);
		...
		Kernel::setMessage("ERROR" , LA::get('cms' , 'error_mail_return_error') . Mail::$error);
		return false;
	}
