<?php namespace Modules\Attractions\Frontend;

/**
 *  Attractions module - frontend model
 *
...
class Attractions
{
    ...
    {
        $route->get('(atrakcje)\/:string-i:id', [
            'module' => 'attractions', 'file' => 'attractions', 'command' => 'view', 'id' => '$3',
        ]);
    }

    public function get_links()
    {
        $this->link['index'] = '/atrakcje';
        $this->link['view']  = '/atrakcje/{name}-i{id}';

        return $this->link;
    }

    protected function create_link($view, $name, $attractions_id)
    {
        if (strlen($name) > 60) {
        ...
        $link = str_replace("{id}", $attractions_id, $link);
    ...
    protected function parse($d, $edit = false)
    {
        ...
        $d['link']             = $this->create_link('view', $d['name'], $d['attractions_id']);
    ...
    {
        $r = Db::query(
            "SELECT a.*, i18.name, i18.address, i18.postcode, i18.city, i18.state_id, i18.phone, i18.email, i18.www, i18.open_hours, s.name as state_name,
            i18.meta_title, i18.meta_description, i18.meta_index, i18.meta_follow
            FROM " . self::$table . " AS a RIGHT JOIN " . self::$table_i18 . " AS i18 ON i18.attractions_id=a.attractions_id
            RIGHT JOIN " . ObjectsStates::$table . " AS s ON s.id=i18.state_id
            WHERE i18.language='" . Language::get_selected() . "' AND a.visibility = 1
            AND a.create_date<=NOW() " . (!empty($exclude_id) ? " AND a.attractions_id != '" . $exclude_id . "'" : "") . "
    ...
    public function get_view($id)
    {
        $r = Db::query_row(
            ...
            FROM " . self::$table . " AS a RIGHT JOIN " . self::$table_i18 . " AS i18 ON i18.attractions_id=a.attractions_id
            RIGHT JOIN " . ObjectsStates::$table . " AS s ON s.id=i18.state_id
            WHERE a.attractions_id='" . $id . "' AND i18.language = '" . Language::get_selected() . "' AND a.visibility=1
            AND (a.create_date<=NOW())
            ORDER BY create_date DESC"
        );
        if (empty($r)) {
