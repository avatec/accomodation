...
class SpecialOffers {
	...
	static protected $table = "special";
	static protected $table_order = "special_order";
	...
	public static function readExpire( $object_id, $special_id )
	{
		...
		return '<label class="label label-info">ważne do ' . $result['expire_date'] . '</label>';
	}

	public static function addExpire( $object_id, $promotion_id, $special_id )
	{
		$days = Promotion::getDays( $promotion_id );
		Payment::log( 'SpecialOffers::addExpire - days: ' . $days );
		Payment::log( print_r($this->get, true ));
		Payment::log( print_r($this->post, true ));
		Payment::log( print_r($_SERVER, true ));
