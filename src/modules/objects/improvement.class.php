...
/**
 * Objects improvement class
 *
 * @package		Modules
 * @subpackage	Objects/Improvement
...
class ObjectsImprovement {
	...
	public function __construct()
	{
		...
		Navigation::submenu('objects' , 'Udogodnienia', "objects/improvement/list/");
		//Kernel::addAdminMenu("objects", "Udogodnienia", "admin/objects/improvement/list/", null, true);
	...
	{
		if(empty(self::$improvements)) {
			self::$improvements = Db::exec("*" , self::$table , "ORDER BY id");
		}

		if(!empty(self::$improvements)) {
			foreach(self::$improvements as $i) {
				if( $id == $i['id'] ) {
					if( $as_html == false ) {
						return $i['name'];
					} else {
