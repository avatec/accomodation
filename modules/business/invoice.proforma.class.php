...
class BusinessInvoiceProforma {

	public static $Error;
	public static $Empty = array( array("id"=>0,"name"=>"Brak danych"));
	protected static $table = "business_invoice_proforma";
	...
	public function __construct()
	{
		...
		//Kernel::addAdminMenu("business", "Faktury Proforma", "admin/business/proforma/list/", null, "business", false);
		Navigation::submenu('business' , 'Faktury Proforma' , 'business/proforma/list/');
		...
		if(!empty($Data)) {
			foreach($Data as $i) {
				...
				if($i['payment'] == "TRUE") {
					if(!empty($json)) {
						foreach($json as $j) {
							$stats['amount_netto'] += $j['price_netto'];
							$stats['amount_vat'] += $j['price_vat'];
							$stats['amount_tax'] += $j['price_netto'] * 0.18;
							$stats['amount_income'] += $j['price_netto'] - ($j['price_netto'] * 0.18);
						}
					}
				} else {
					if(!empty($json)) {
						foreach($json as $j) {
							$stats['amount_netto_notpay'] += $j['price_netto'];
							$stats['amount_vat_notpay'] += $j['price_vat'];
							$stats['amount_tax_notpay'] += $j['price_netto'] * 0.18;
							$stats['amount_income_notpay'] += $j['price_netto'] - ($j['price_netto'] * 0.18);
						}
					}
				}
			}
		...
		if( is_null($id) ) {
			...
			if(!empty($Result)) {
				foreach($Result as $k=>$i) {
					...
					$Result[$k]['pdf_file'] = file_exists( $app_path . "archive/" . $year . "/faktura_proforma_" . $filename . ".pdf" );
	...
	public function create()
	{
		...
		" . (!empty($request->post['sell_date']) ? "'" . $request->post['sell_date'] . "'" : "NULL") . ",
	...
	public function save( $id )
	{
		...
		" . (!empty($request->post['sell_date']) ? "sell_date = '" . $request->post['sell_date'] . "'," : "") . "
	...
	{
		Db::update(self::$table , "cancel='TRUE'" , "id='" . $id . "'");
		Kernel::setMessage("NOTICE" , "Anulowano fakturę");
		return true;
	}
