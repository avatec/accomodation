...
/**
 * Objects distance class
 *
 * @package		Modules
 * @subpackage	Objects/Distance
...
class ObjectsDistance {
	...
	public function __construct()
	{
		...
		Navigation::submenu('objects' , 'Odległości', "objects/distance/list/");
		//Kernel::addAdminMenu("objects", "Odległości", "admin/objects/distance/list/", null, true);
	...
	{
		if(empty(self::$distance)) {
			self::$distance = Db::exec("*" , self::$table , "ORDER BY id");
		}

		if(!empty(self::$distance)) {
			foreach(self::$distance as $i) {
				if( $id == $i['id'] ) {
					return $i['name'];
				}
			}
	...
	public function save( $id )
	{
		...
		if(!empty($result)) {
			...
			self::$Error = Db::error();
			Kernel::setMessage("ERROR" , "Wystąpił błąd w bazie danych podczas operacji:<br/>" . self::$Error);
			return false;
		}
